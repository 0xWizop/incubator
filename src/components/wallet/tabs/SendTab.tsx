...
import { ArrowUpRight, ChevronDown, AlertTriangle, Clock, Loader2, CheckCircle, XCircle, ExternalLink, BookUser, Check } from 'lucide-react';
...
export function SendTab() {
    ...
    // Address Book
    const { addresses, getAddressByAddress, markAsUsed, addAddress } = useAddressBook();
    const savedAddress = recipient ? getAddressByAddress(recipient) : undefined;
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (result.success) {
                // Store values for modal display
    ...
    return (
        <>
            ...
            <AddressBookModal
                ...
                mode="select"
                currentChain={activeChain as any}
            />
            ...
            <TransactionSuccessModal
                ...
                explorerUrl={txResult?.hash ? getExplorerUrl(txResult.hash) : ''}
                type="sent"
            />

            <div className="space-y-4">
                {/* Transaction Error (only show errors, success uses modal) */}
                {txResult && !txResult.success && (
                    <div className="p-4 rounded-xl border flex items-start gap-3 bg-[var(--accent-red)]/10 border-[var(--accent-red)]/20">
                ...
                <div className="p-4 rounded-2xl bg-[var(--background-tertiary)] border border-[var(--border)]">
                    <div className="flex justify-between items-center mb-2">
                        <label className="text-xs text-[var(--foreground-muted)]">Recipient address</label>
                        <button
                    ...
                    <div className="flex items-center gap-2">
                        <input
                            ...
                            disabled={isSending}
                            className={clsx(
                                'flex-1 bg-transparent text-sm font-mono outline-none placeholder:text-[var(--foreground-muted)]/30 disabled:opacity-50',
                                isValidAddress === false && 'text-[var(--accent-red)]'
                            )}
                        />
                        {savedAddress && (
                            <span className="flex items-center gap-1 px-2 py-0.5 text-[10px] font-medium rounded-full bg-[var(--accent-green)]/10 text-[var(--accent-green)]">
                    ...
                    {isValidAddress && !savedAddress && (
                        <p className="text-xs text-[var(--accent-yellow)] mt-2 flex items-center gap-1">
                            <AlertTriangle className="w-3 h-3" />
